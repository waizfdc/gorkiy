...
final class OpenSSLX509CRLEntry extends X509CRLEntry {
    ...
    public boolean hasUnsupportedCriticalExtension() {
        for (String X509_REVOKED_get_ext : NativeCrypto.get_X509_REVOKED_ext_oids(this.mContext, 1)) {
            if (NativeCrypto.X509_supported_extension(NativeCrypto.X509_REVOKED_get_ext(this.mContext, X509_REVOKED_get_ext)) != 1) {
                return true;
            }
        }
        return false;
