...
public class TransitionSet extends Transition {
    ...
    public TransitionSet addTransition(Transition transition) {
        addTransitionInternal(super);
        if (this.mDuration >= 0) {
            super.setDuration(this.mDuration);
        }
        if ((this.mChangeFlags & 1) != 0) {
            super.setInterpolator(getInterpolator());
        }
        if ((this.mChangeFlags & 2) != 0) {
            super.setPropagation(getPropagation());
        }
        if ((this.mChangeFlags & 4) != 0) {
            super.setPathMotion(getPathMotion());
        }
        if ((this.mChangeFlags & 8) != 0) {
            super.setEpicenterCallback(getEpicenterCallback());
        }
        return this;
    }
    ...
    public void createAnimators(ViewGroup viewGroup, TransitionValuesMaps transitionValuesMaps, TransitionValuesMaps transitionValuesMaps2, ArrayList<TransitionValues> arrayList, ArrayList<TransitionValues> arrayList2) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (startDelay > 0 && (this.mPlayTogether || i == 0)) {
                long startDelay2 = super.getStartDelay();
                if (startDelay2 > 0) {
                    super.setStartDelay(startDelay2 + startDelay);
                } else {
                    super.setStartDelay(startDelay);
                }
            }
            super.createAnimators(viewGroup, transitionValuesMaps, transitionValuesMaps2, arrayList, arrayList2);
        }
    }
    ...
    public void captureStartValues(TransitionValues transitionValues) {
        if (isValidTarget(transitionValues.view)) {
            ...
            while (it.hasNext()) {
                ...
                if (super.isValidTarget(transitionValues.view)) {
