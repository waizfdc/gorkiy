...
public final class BufferKt {
    ...
    public static final String commonReadUtf8LineStrict(Buffer buffer, long j) {
        ...
        if (j >= 0) {
            ...
            throw new EOFException("\\n not found: limit=" + Math.min(buffer.size(), j) + " content=" + buffer2.readByteString().hex() + ((char) Typography.ellipsis));
        }
    ...
    public static final void commonWrite(Buffer buffer, Buffer buffer2, long j) {
        Segment segment;
        Intrinsics.checkParameterIsNotNull(buffer, "$this$commonWrite");
        Intrinsics.checkParameterIsNotNull(buffer2, FirebaseAnalytics.Param.SOURCE);
        if (buffer2 != buffer) {
