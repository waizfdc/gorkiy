...
public final class CompletableDelay extends Completable {
    ...
    static final class Delay extends AtomicReference<Disposable> implements CompletableObserver, Runnable, Disposable {
        ...
        public void onSubscribe(Disposable disposable) {
            if (DisposableHelper.setOnce(super, disposable)) {
                this.downstream.onSubscribe(this);
            }
        }

        public void onComplete() {
            DisposableHelper.replace(super, this.scheduler.scheduleDirect(this, this.delay, this.unit));
        }

        public void onError(Throwable th) {
            this.error = th;
            DisposableHelper.replace(super, this.scheduler.scheduleDirect(this, this.delayError ? this.delay : 0, this.unit));
        }
