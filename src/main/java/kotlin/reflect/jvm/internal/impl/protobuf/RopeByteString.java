...
class RopeByteString extends ByteString {
    ...
    private RopeByteString(ByteString byteString, ByteString byteString2) {
        ...
        this.totalLength = size + super.size();
        this.treeDepth = Math.max(super.getTreeDepth(), super.getTreeDepth()) + 1;
    }

    static ByteString concatenate(ByteString byteString, ByteString superR) {
        ...
        if (super.size() != 0) {
            ...
            if (size < 128) {
                return concatenateBytes(super, super);
            }
            if (ropeByteString != null && ropeByteString.right.size() + super.size() < 128) {
                superR = new RopeByteString(ropeByteString.left, concatenateBytes(ropeByteString.right, super));
            } else if (ropeByteString == null || ropeByteString.left.getTreeDepth() <= ropeByteString.right.getTreeDepth() || ropeByteString.getTreeDepth() <= super.getTreeDepth()) {
                if (size >= minLengthByDepth[Math.max(super.getTreeDepth(), super.getTreeDepth()) + 1]) {
                    return new RopeByteString(super, super);
                }
                return new Balancer().balance(super, super);
            } else {
                superR = new RopeByteString(ropeByteString.left, new RopeByteString(ropeByteString.right, super));
            }
        }
        return superR;
    }
    ...
    private static LiteralByteString concatenateBytes(ByteString byteString, ByteString byteString2) {
        ...
        super.copyTo(bArr, 0, 0, size);
        super.copyTo(bArr, 0, size, size2);
    ...
    public boolean isValidUtf8() {
        ...
        if (super.partialIsValidUtf8(partialIsValidUtf8, 0, super.size()) == 0) {
            return true;
        }
        return false;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (this.totalLength != super.size()) {
            return false;
        }
        if (this.totalLength == 0) {
            return true;
        }
        if (this.hash == 0 || (peekCachedHashCode = super.peekCachedHashCode()) == 0 || this.hash == peekCachedHashCode) {
            return equalsFragments(super);
        }
        return false;
    }

    private boolean equalsFragments(ByteString byteString) {
        PieceIterator pieceIterator = new PieceIterator(super);
        LiteralByteString literalByteString = (LiteralByteString) pieceIterator.next();
        PieceIterator pieceIterator2 = new PieceIterator(super);
